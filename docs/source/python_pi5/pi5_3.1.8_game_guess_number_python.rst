...
Guessing Numbers is an entertaining party game where you and your 
friends take turns inputting a number between 0 and 99. The range 
of possible numbers narrows down with each subsequent input until 
a player correctly guesses the hidden target number. The player 
who guesses the correct number is deemed the "unlucky one" and receives 
a playful punishment. For instance, if the secret number is 51 
(unknown to the players), and player 1 inputs 50, the prompt will 
indicate that the number range is now 50 to 99. If player 2 then 
inputs 70, the range narrows further to 50 to 70. If player 3 correctly 
guesses 51, they become the "unlucky" player. In this project, we will 
use a keypad for number input and an LCD screen to display the outcomes 
and number ranges.
...
.. image:: ../python_pi5/img/list/4.1.17_game_guess_number_list.png
    :width: 800
    :align: center
...
.. image:: ../python_pi5/img/schematic/4.1.17_game_guess_number_schematic.png
   :align: center
...
.. image:: ../python_pi5/img/connect/3.1.8.png

**Step 2**: Setup I2C (see  `I2C Config <../Appendix/i2c_configuration.html>`_.)
...
.. code-block:: 
    ...
    sudo python3 3.1.8_GAME_GuessNumber_zero.py
